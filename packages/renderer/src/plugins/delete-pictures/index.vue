    ...
    <div class="plugin-specific-content">
      ...
      <a-card v-if="currentStep === 1" title="步骤2：设置规则" class="step-card">
        <a-form layout="vertical">
          ...
          <a-form-item v-if="deleteScope === 'selected'" label="工作表名称">
            <a-input
              v-model:value="sheetName"
              placeholder="请输入要删除图片的工作表名称"
            />
          </a-form-item>
        </a-form>
      </a-card>
...
// 插件基本信息
const pluginTitle = ref('删除 Excel 中的图片')
const pluginDescription = ref('批量删除 Excel 文档中的图片，可以删除指定工作表或整个工作簿中的图片')
...
delete_scope = ${JSON.stringify(deleteScope.value)}
...
elif delete_scope == 'selected' and sheet_name:
    # 删除指定工作表中的图片
...
result = {
    'status': 'success',
    'message': f'成功删除 {removed_count} 张图片',
    'file_name': file_name
}
