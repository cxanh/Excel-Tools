  ...
  >
    ...
    <div class="plugin-specific-content">
      ...
      <a-card v-if="currentStep === 0" title="步骤1：选择文件" class="step-card">
        <p>请选择要移除空行的 Excel 文件</p>
      </a-card>
      
      <!-- 步骤2：设置规则 -->
      <a-card v-if="currentStep === 1" title="步骤2：设置规则" class="step-card">
        <a-form layout="vertical">
          <a-form-item label="空行定义">
            <a-radio-group v-model:value="emptyRowDefinition">
              <a-radio value="all-empty">所有单元格为空</a-radio>
              <a-radio value="main-columns-empty">主要列为空</a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item v-if="emptyRowDefinition === 'main-columns-empty'" label="主要列（用逗号分隔）">
            <a-input
              v-model:value="mainColumns"
      ...
      <a-card v-if="currentStep === 2" title="步骤3：其他选项" class="step-card">
        <a-form layout="vertical">
          <a-form-item label="处理选项">
            ...
            <a-checkbox v-model:checked="removeHiddenRows">同时移除隐藏行</a-checkbox>
          </a-form-item>
        </a-form>
      </a-card>
    ...
    <template #help-content>
      <div class="help-documentation">
        ...
        <ol>
          <li>上传 Excel 文件</li>
          <li>设置空行定义（所有单元格为空或主要列为空）</li>
          <li>设置是否备份原始文件和是否移除隐藏行</li>
          <li>选择输出文件夹</li>
          <li>开始处理</li>
        </ol>
...
// 插件基本信息
const pluginTitle = ref('移除 Excel 空行')
const pluginDescription = ref('移除 Excel 文件中的空行，清理数据')
...
// 处理规则
const emptyRowDefinition = ref('all-empty')
const mainColumns = ref('')
const backupOriginal = ref(true)
const removeHiddenRows = ref(false)
...
empty_row_def = ${JSON.stringify(emptyRowDefinition.value)}
main_columns = ${JSON.stringify(mainColumns.value)}
backup_original = ${JSON.stringify(backupOriginal.value)}
remove_hidden = ${JSON.stringify(removeHiddenRows.value)}
...
for sheet_name in wb.sheetnames:
    ...
    for row_idx in range(len(rows), 0, -1):
        ...
        # 检查是否是隐藏行
        if remove_hidden and ws.row_dimensions[row_idx].hidden:
        ...
        if empty_row_def == 'all-empty':
            # 检查所有单元格是否为空
            for cell in row:
                if cell.value is not None and str(cell.value).strip() != '':
                    is_empty = False
                    break
        elif empty_row_def == 'main-columns-empty' and main_columns:
            # 检查主要列是否为空
            cols = main_columns.split(',')
            main_cells = []
            for col in cols:
                col = col.strip()
                if col.isalpha():
                ...
                if col_idx <= len(row):
                    main_cells.append(row[col_idx - 1])
...
result = {
    'status': 'success',
    'message': f'成功移除 {removed_rows_count} 行',
    'file_name': file_name
}
