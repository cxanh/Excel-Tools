<template>
  <div class="plugin-container">
    ...
    <div v-else-if="currentStep === 1" class="step-content">
      <h3 class="step-title">设置删除选项</h3>
      <a-divider />

      <a-card title="删除设置" class="setting-card">
...
const pluginTitle = '删除 Excel 页眉页脚';
...
const startProcess = async () => {
  ...
  try {
    // 获取Python脚本
    const script = await getPythonScript('delete-header-footer');
    if (!script) {
