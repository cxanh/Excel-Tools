  ...
  >
    <template #default="{ currentStep }">
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="优化和压缩Excel文件，减小文件大小。包括压缩图片、删除未使用样式、优化公式等。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="优化选项" :bordered="false">
          <a-form layout="vertical">
            <a-form-item>
              <a-checkbox v-model:checked="compressImages">压缩图片</a-checkbox>
            </a-form-item>
            <a-form-item v-if="compressImages" label="图片质量">
              <a-slider
            ...
            <a-form-item>
              <a-checkbox v-model:checked="removeUnusedStyles"
                >删除未使用样式</a-checkbox
              >
            </a-form-item>
            <a-form-item>
              <a-checkbox v-model:checked="optimizeFormulas"
                >优化公式</a-checkbox
              >
            </a-form-item>
          </a-form>
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        <div>
                          原大小: {{ formatFileSize(files[index]?.size) }}
                        </div>
                        <div>优化后: {{ formatFileSize(item.size) }}</div>
                        <div>压缩率: {{ item.compressionRate }}%</div>
                      </div>
...
const removeUnusedStyles = ref(true);
const optimizeFormulas = ref(true);
  ...
  try {
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const result = await runPythonScript("optimize-excel", {
          fileData: fileData.split(",")[1],
          compressImages: compressImages.value,
          imageQuality: imageQuality.value,
          removeUnusedStyles: removeUnusedStyles.value,
          optimizeFormulas: optimizeFormulas.value,
        });
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, "_optimized.xlsx");
