<template>
  <div class="plugin-container">
    ...
    <div v-else-if="currentStep === 1" class="step-content">
      <h3 class="step-title">设置页眉页脚</h3>
      <a-divider />

      <a-card title="页眉设置" class="setting-card">
        <a-checkbox v-model:checked="settings.header.enabled">启用页眉</a-checkbox>
        
        <a-form layout="vertical" v-if="settings.header.enabled">
          <a-row :gutter="[16, 16]">
            <a-col :span="8">
              <a-form-item label="左页眉">
                <a-input
                  v-model:value="settings.header.left"
                  placeholder="左页眉内容"
                />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item label="中页眉">
                <a-input
                  v-model:value="settings.header.center"
                  placeholder="中页眉内容"
                />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item label="右页眉">
                <a-input
                  v-model:value="settings.header.right"
                  placeholder="右页眉内容"
                />
              </a-form-item>
            </a-col>
      ...
      <a-card title="页脚设置" class="setting-card" :style="{ marginTop: '20px' }">
        <a-checkbox v-model:checked="settings.footer.enabled">启用页脚</a-checkbox>
        
        <a-form layout="vertical" v-if="settings.footer.enabled">
          <a-row :gutter="[16, 16]">
            <a-col :span="8">
              <a-form-item label="左页脚">
                <a-input
                  v-model:value="settings.footer.left"
                  placeholder="左页脚内容"
                />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item label="中页脚">
                <a-input
                  v-model:value="settings.footer.center"
                  placeholder="中页脚内容"
                />
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item label="右页脚">
                <a-input
                  v-model:value="settings.footer.right"
                  placeholder="右页脚内容"
                />
              </a-form-item>
            </a-col>
      ...
      <a-card title="工作表选择" class="setting-card" :style="{ marginTop: '20px' }">
...
const pluginTitle = '添加/修改 Excel 页眉页脚';
...
const startProcess = async () => {
  ...
  try {
    // 获取Python脚本
    const script = await getPythonScript('add-header-footer');
    if (!script) {
