  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="从Excel文件中提取所有嵌入的图片，并保存为独立的图片文件。支持批量处理多个Excel文件，输出为ZIP压缩包。"
          type="info"
          show-icon
        ...
        <a-card title="处理规则配置" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="图片格式">
              <a-radio-group v-model:value="formData.imageFormat">
                <a-radio value="original">保持原格式</a-radio>
                <a-radio value="png">转换为PNG</a-radio>
                <a-radio value="jpg">转换为JPG</a-radio>
              </a-radio-group>
            </a-form-item>

            <a-form-item label="文件命名">
              <a-radio-group v-model:value="formData.namingPattern">
                <a-radio value="sequential"
                  >顺序编号 (image_1, image_2...)</a-radio
                >
                <a-radio value="sheet"
                  >工作表名称 (Sheet1_1, Sheet1_2...)</a-radio
                >
                <a-radio value="position">单元格位置 (A1, B2...)</a-radio>
              </a-radio-group>
            </a-form-item>

            <a-form-item>
              <a-checkbox v-model:checked="formData.createSubfolders">
                为每个Excel文件创建独立文件夹
              </a-checkbox>
            </a-form-item>

            <a-form-item>
              <a-checkbox v-model:checked="formData.includeMetadata">
                生成图片信息清单（JSON）
              </a-checkbox>
            </a-form-item>
          </a-form>
        ...
        <a-card title="导出选项" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="输出格式">
              <a-radio-group v-model:value="exportOptions.format" disabled>
                <a-radio value="zip">ZIP压缩包</a-radio>
              </a-radio-group>
              <template #extra>图片将打包为ZIP压缩包便于下载</template>
            </a-form-item>
            <a-form-item label="文件命名">
                ...
                <a-card size="small">
                  ...
                  <div v-if="item.success && item.statistics">
                    <a-descriptions :column="2" size="small">
                      <a-descriptions-item label="提取图片数"
                        ><a-tag color="green">{{
                          item.statistics.imagesExtracted
                      ...
                      <a-descriptions-item label="总大小">{{
                        formatSize(item.statistics.totalSize)
                      }}</a-descriptions-item>
                    ...
                    <div
                      v-if="
                        item.statistics.imageList &&
                        item.statistics.imageList.length > 0
                      "
                    ...
                    >
                      ...
                      <a-space wrap>
                        <a-tag
                          v-for="(img, idx) in item.statistics.imageList.slice(
                            0,
                            10,
                          )"
                          :key="idx"
                        ...
                        <a-tag
                          v-if="item.statistics.imageList.length > 10"
                          color="default"
                        >
                          ...还有 {{ item.statistics.imageList.length - 10 }} 个
                        </a-tag>
                      </a-space>
                    </div>
                  </div>
...
const formData = ref({
  imageFormat: "original" as "original" | "png" | "jpg",
  namingPattern: "sequential" as "sequential" | "sheet" | "position",
...
async function handleDownload(result: ProcessResult) {
  try {
    const zipFileName = result.fileName.replace(/\.[^.]+$/, "_images.zip");
    await downloadResult(result, zipFileName);
    message.success("压缩包已保存");
  } catch (error) {
