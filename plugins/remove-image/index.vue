  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="删除Excel文件中的所有图片，包括嵌入的图片和图表。删除后可以显著减小文件大小。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="处理选项" :bordered="false">
          <a-alert
            message="简单处理"
            description="此功能无需额外配置，将自动删除所有图片。"
            type="info"
            show-icon
          />
        </a-card>
                ...
                <a-card size="small">
                  ...
                  <div v-if="item.success && item.statistics">
                    <a-descriptions :column="2" size="small">
                      <a-descriptions-item label="删除图片数"
                        ><a-tag color="red">{{
                          item.statistics.imagesDeleted
                        }}</a-tag></a-descriptions-item
                      >
                      <a-descriptions-item label="原始大小">{{
                        formatFileSize(item.statistics.originalSize)
                      }}</a-descriptions-item>
                      <a-descriptions-item label="处理后大小">{{
                        formatFileSize(item.statistics.newSize)
                      }}</a-descriptions-item>
                      <a-descriptions-item label="减小"
                        ><a-tag color="green">{{
                          item.statistics.sizeReduction
                        }}</a-tag></a-descriptions-item
                      >
...
const exportOptions = ref({ naming: "suffix", suffix: "_no_images" });
...
async function handleProcess() {
  ...
  try {
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      try {
        const result = await processFile(file, props.workerScript);
