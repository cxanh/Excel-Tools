  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="删除Excel文件的所有页眉页脚内容，保留其他打印设置（如页边距、纸张大小）。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="处理规则配置" :bordered="false">
          <a-form layout="vertical">
            ...
            <a-alert
              message="提示"
              description="此操作将清除所有页眉页脚内容，但保留其他打印设置（如页边距、纸张大小等）。"
              type="warning"
              show-icon
            />
          </a-form>
        ...
        <a-card title="导出选项" :bordered="false">
          <a-form layout="vertical">
            ...
            <a-form-item v-if="exportOptions.naming === 'suffix'" label="后缀内容">
              <a-input v-model:value="exportOptions.suffix" placeholder="例如：_no_header" />
            </a-form-item>
          </a-form>
        </a-card>
      </div>
                ...
                <a-card size="small">
                  ...
                  <div v-if="item.success && item.statistics">
                    <a-descriptions :column="3" size="small">
                      ...
                      <a-descriptions-item label="有页眉页脚">{{ item.statistics.sheetsWithHeaderFooter }}</a-descriptions-item>
                      <a-descriptions-item label="已清除"><a-tag color="red">{{ item.statistics.sheetsProcessed }}</a-tag></a-descriptions-item>
                    </a-descriptions>
                  </div>
...
const formData = ref({ sheets: ['all'], apply_to_all: true })
const exportOptions = ref({ naming: 'suffix', suffix: '_no_header' })
