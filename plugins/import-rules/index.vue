  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="从文件导入批量修改规则并应用到Excel，支持JSON和CSV格式的规则文件。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="上传规则文件" :bordered="false">
          <a-alert
            message="规则文件格式"
            description="支持JSON或CSV格式。JSON格式示例: [{column: 'A', row: 1, value: '新值'}]"
            type="info"
            show-icon
            style="margin-bottom: 16px"
          />
          <a-upload
            :before-upload="handleRulesUpload"
          ...
          <div v-if="rulesFile" style="margin-top: 16px">
            <a-tag color="green">{{ rulesFile.name }}</a-tag>
            <a-button type="link" size="small" danger @click="removeRulesFile">移除</a-button>
          </div>
        </a-card>
      </div>
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        <div>已应用 {{ item.rulesApplied }} 条规则</div>
                      </div>
...
const canProceed = computed(() => {
  ...
  if (step === 1) return rulesFile.value !== null
  return true
})
  ...
  try {
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const result = await runPythonScript('import-rules', {
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, '_modified.xlsx')
