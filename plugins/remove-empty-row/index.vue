  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="删除Excel文件中的空行，支持智能识别完全空行和部分空行。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="处理规则配置" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="空行识别模式">
              <a-radio-group v-model:value="formData.emptyRowMode" button-style="solid">
                <a-radio-button value="complete">完全空行</a-radio-button>
                <a-radio-button value="partial">部分空行</a-radio-button>
              </a-radio-group>
            </a-form-item>
            <a-form-item v-if="formData.emptyRowMode === 'partial'" label="空值阈值">
              <a-slider v-model:value="formData.emptyThreshold" :min="0" :max="100" :marks="{ 0: '0%', 50: '50%', 100: '100%' }" />
              <div style="margin-top: 8px">当行中空值单元格比例超过 {{ formData.emptyThreshold }}% 时视为空行</div>
            </a-form-item>
          </a-form>
        </a-card>
      </div>
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        ...
                        <div>处理后: {{ formatFileSize(item.size) }}</div>
                        <div>删除空行数: {{ item.emptyRowsDeleted }}</div>
                      </div>
    ...
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const params = {
          ...
          emptyRowMode: formData.value.emptyRowMode,
          emptyThreshold: formData.value.emptyThreshold
        }
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, '_no_empty_rows.xlsx')
