  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="将大型CSV文件拆分成多个小文件，支持按行数或文件大小拆分。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="拆分选项" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="拆分方式">
              <a-radio-group v-model:value="splitMode">
                <a-radio value="rows">按行数拆分</a-radio>
                <a-radio value="size">按文件大小拆分</a-radio>
              </a-radio-group>
            </a-form-item>
            <a-form-item v-if="splitMode === 'rows'" label="每个文件行数">
              <a-input-number v-model:value="rowsPerFile" :min="1" :max="1000000" style="width: 200px" />
            </a-form-item>
            <a-form-item v-if="splitMode === 'size'" label="每个文件大小(MB)">
              <a-input-number v-model:value="sizePerFile" :min="1" :max="100" style="width: 200px" />
            </a-form-item>
            <a-form-item>
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        <div>已拆分为 {{ item.fileCount }} 个文件</div>
                      </div>
...
const splitMode = ref('rows')
const rowsPerFile = ref(10000)
const sizePerFile = ref(10)
    ...
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const params = {
          ...
          sizePerFile: sizePerFile.value,
          includeHeader: includeHeader.value
        }

        const result = await runPythonScript('csv-split', params)
