  ...
  >
    <template #default="{ currentStep }">
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="清空Excel的元数据，保护隐私信息。包括作者、公司、修改日期等信息。"
          type="info"
          show-icon
...
import { LoadingOutlined, CheckCircleOutlined, FileExcelOutlined } from '@ant-design/icons-vue'
  ...
  try {
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const result = await runPythonScript('clear-metadata', {
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, '_no_metadata.xlsx')
