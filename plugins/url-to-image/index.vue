  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="将Excel单元格中的图片URL地址转换为实际的嵌入图片。支持http/https链接，图片将下载并插入到对应单元格位置。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="处理规则配置" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="URL列">
              <a-input
                v-model:value="formData.urlColumn"
                placeholder="例如: A (包含图片URL的列)"
              />
              <template #extra>指定包含图片URL的列，例如A、B、C等</template>
            </a-form-item>

            <a-form-item label="插入位置">
              <a-radio-group v-model:value="formData.insertPosition">
                <a-radio value="same">同一单元格</a-radio>
                <a-radio value="next">下一列</a-radio>
                <a-radio value="custom">自定义列</a-radio>
              </a-radio-group>
            </a-form-item>

            <a-form-item
              v-if="formData.insertPosition === 'custom'"
              label="目标列"
            >
              <a-input
            ...
            <a-form-item label="图片尺寸">
              ...
              <template #extra>设置插入图片的宽度和高度（像素）</template>
            </a-form-item>

            <a-form-item>
              <a-checkbox v-model:checked="formData.clearUrl">
                转换后清空URL单元格
              </a-checkbox>
            </a-form-item>

            <a-form-item>
              <a-checkbox v-model:checked="formData.skipErrors">
                跳过无效URL继续处理
              </a-checkbox>
            </a-form-item>
          </a-form>
                ...
                <a-card size="small">
                  ...
                  <div v-if="item.success && item.statistics">
                    <a-descriptions :column="2" size="small">
                      <a-descriptions-item label="转换图片数"
                        ><a-tag color="green">{{
                          item.statistics.imagesConverted
                        }}</a-tag></a-descriptions-item
                      >
                      <a-descriptions-item label="失败数"
                        ><a-tag
                          :color="
                            item.statistics.failedUrls > 0 ? 'red' : 'default'
                          "
                          >{{ item.statistics.failedUrls }}</a-tag
                        ></a-descriptions-item
                      >
...
const formData = ref({
  urlColumn: "A",
  insertPosition: "next" as "same" | "next" | "custom",
  targetColumn: "B",
  imageWidth: 100,
  imageHeight: 100,
  clearUrl: false,
...
const exportOptions = ref({ naming: "suffix", suffix: "_with_images" });
...
const canProceed = computed(() => {
  ...
  if (step === 1) {
    if (!formData.value.urlColumn.trim()) return false;
    if (
      formData.value.insertPosition === "custom" &&
      !formData.value.targetColumn.trim()
    )
      return false;
    return true;
  }
...
async function handleProcess() {
  ...
  try {
    ...
    const params = {
      urlColumn: formData.value.urlColumn.toUpperCase(),
      insertPosition: formData.value.insertPosition,
      targetColumn: formData.value.targetColumn?.toUpperCase() || "",
      imageWidth: formData.value.imageWidth,
      imageHeight: formData.value.imageHeight,
      clearUrl: formData.value.clearUrl,
      skipErrors: formData.value.skipErrors,
    };
