  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="删除Excel文件中的所有宏代码(VBA)，生成不含宏的标准Excel文件。适用于安全性要求较高的场景。"
          type="info"
          show-icon
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        <div>
                          原文件大小: {{ formatFileSize(files[index]?.size) }}
                        </div>
                        <div>处理后大小: {{ formatFileSize(item.size) }}</div>
                        <div>宏已删除，文件已转换为标准格式</div>
                      </div>
  ...
  try {
    ...
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        // 调用Python脚本处理
        const result = await runPythonScript("remove-macro", {
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.xlsm?$/i, "_no_macro.xlsx");
