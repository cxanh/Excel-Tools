  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="识别并删除Excel文件中的重复数据行，保留首次出现的行。支持全行比较和指定列比较两种模式。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="处理规则配置" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="比较模式">
              <a-radio-group v-model:value="formData.compareMode" button-style="solid">
                <a-radio-button value="all">全行比较</a-radio-button>
                <a-radio-button value="columns">指定列比较</a-radio-button>
              </a-radio-group>
            </a-form-item>
            <a-form-item v-if="formData.compareMode === 'columns'" label="比较列号">
              <a-input
                v-model:value="formData.compareColumns"
                placeholder="例如：1,2,3 表示比较第1、2、3列"
              />
              <template #extra>输入要比较的列号，多个列用逗号分隔</template>
            </a-form-item>
          </a-form>
        </a-card>
      </div>
        ...
        <a-card title="导出选项" :bordered="false">
          <a-form layout="vertical">
            ...
            <a-form-item v-if="exportOptions.naming === 'suffix'" label="后缀内容">
              <a-input v-model:value="exportOptions.suffix" placeholder="例如：_no_duplicates" />
            </a-form-item>
          </a-form>
        </a-card>
      </div>
                ...
                <a-card size="small">
                  ...
                  <div v-if="item.success && item.statistics">
                    <a-descriptions :column="3" size="small">
                      <a-descriptions-item label="总行数">{{ item.statistics.totalRows }}</a-descriptions-item>
                      <a-descriptions-item label="重复行数"><a-tag color="red">{{ item.statistics.duplicateRows }}</a-tag></a-descriptions-item>
...
const formData = ref({ compareMode: 'all', compareColumns: '' })
const exportOptions = ref({ naming: 'suffix', suffix: '_no_duplicates' })
...
const canProceed = computed(() => {
  ...
  if (step === 1 && formData.value.compareMode === 'columns') return formData.value.compareColumns.trim() !== ''
...
function handleClearAll() {
  files.value = []
  results.value = []
  formData.value = { compareMode: 'all', compareColumns: '' }
...
async function handleProcess() {
  ...
  try {
    ...
    const params = {
      compareMode: formData.value.compareMode,
      compareColumns: formData.value.compareMode === 'columns' 
        ? formData.value.compareColumns.split(',').map(c => parseInt(c.trim()))
        : []
    }
