  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="在Excel的指定位置插入新工作表，支持自定义工作表名称和插入位置。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="插入选项" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="新工作表名称">
              <a-input v-model:value="sheetName" placeholder="例如：新数据" />
            </a-form-item>
            <a-form-item label="插入位置">
              <a-radio-group v-model:value="insertPosition">
                ...
                <a-radio value="custom">指定位置</a-radio>
              </a-radio-group>
            </a-form-item>
            <a-form-item v-if="insertPosition === 'custom'" label="位置索引">
              <a-input-number
...
const sheetName = ref("新工作表");
const insertPosition = ref("end");
...
const canProceed = computed(() => {
  ...
  if (step === 1) return sheetName.value.trim() !== "";
  return true;
});
    ...
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const params = {
          ...
          customIndex: customIndex.value,
        };

        const result = await runPythonScript("insert-sheet", params);
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, "_with_sheet.xlsx");
