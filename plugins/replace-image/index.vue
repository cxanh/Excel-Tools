  ...
  >
    <template #default="{ currentStep }">
      ...
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="替换Excel文件中的图片，支持批量替换和指定位置替换。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="上传替换图片" :bordered="false">
          <a-upload
          ...
          <div v-if="replacementImage" style="margin-top: 16px">
            <a-tag color="green">{{ replacementImage.name }}</a-tag>
            <a-button
              type="link"
              size="small"
              danger
              @click="removeReplacementImage"
              >移除</a-button
            >
          </div>
        </a-card>
                  ...
                  <a-list-item-meta>
                    ...
                    <template #description>
                      <div v-if="item.success">
                        ...
                        <div>替换后: {{ formatFileSize(item.size) }}</div>
                        <div>替换图片数: {{ item.imagesReplaced }}</div>
                      </div>
...
const replacementImage = ref<File | null>(null);
const replacementImageData = ref("");
...
const canProceed = computed(() => {
  ...
  if (step === 1) return replacementImage.value !== null;
  return true;
});
...
const handleClearAll = () => {
  files.value = [];
  results.value = [];
  replacementImage.value = null;
  replacementImageData.value = "";
  if (fileUploadRef.value) {
...
const handleImageUpload = async (file: File) => {
  replacementImage.value = file;
  const reader = new FileReader();
  reader.onload = () => {
    replacementImageData.value = reader.result as string;
  };
  reader.readAsDataURL(file);
  message.success("替换图片已上传");
  return false;
};
    ...
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const params = {
          ...
          replacementImage: replacementImageData.value.split(",")[1],
        };

        const result = await runPythonScript("replace-image", params);
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const fileName = file.name.replace(/\.[^.]+$/, "_replaced.xlsx");
