  ...
  >
    <template #default="{ currentStep }">
      <div v-if="currentStep === 0" class="step-content">
        <a-alert
          message="功能说明"
          description="添加或删除Excel的保护设置，支持工作簿保护和工作表保护。"
          type="info"
          show-icon
      ...
      <div v-if="currentStep === 1" class="step-content">
        <a-card title="保护设置" :bordered="false">
          <a-form layout="vertical">
            <a-form-item label="操作类型">
              <a-radio-group v-model:value="action">
                <a-radio value="add">添加保护</a-radio>
                <a-radio value="remove">删除保护</a-radio>
              </a-radio-group>
            </a-form-item>
            <a-form-item v-if="action === 'add'" label="密码">
              <a-input-password
                v-model:value="password"
                placeholder="输入保护密码"
              />
            </a-form-item>
            <a-form-item v-if="action === 'add'">
              <a-checkbox v-model:checked="protectWorkbook"
                >保护工作簿</a-checkbox
              >
            </a-form-item>
            <a-form-item v-if="action === 'add'">
              <a-checkbox v-model:checked="protectSheets"
                >保护所有工作表</a-checkbox
              >
            </a-form-item>
          </a-form>
...
const password = ref("");
const protectWorkbook = ref(true);
const protectSheets = ref(true);
  ...
  try {
    for (let i = 0; i < files.value.length; i++) {
      ...
      try {
        ...
        const result = await runPythonScript("manage-protection", {
          fileData: fileData.split(",")[1],
          action: action.value,
          password: password.value,
          protectWorkbook: protectWorkbook.value,
          protectSheets: protectSheets.value,
        });
...
const downloadFile = (index: number) => {
  ...
  if (result.success && result.data) {
    const suffix = action.value === "add" ? "_protected" : "_unprotected";
    const fileName = file.name.replace(/\.[^.]+$/, `${suffix}.xlsx`);
