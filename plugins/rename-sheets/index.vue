<template>
  <div class="plugin-container">
    ...
    <div v-else-if="currentStep === 1" class="step-content">
      <h3 class="step-title">设置重命名规则</h3>
      <a-divider />

      <a-card title="工作表重命名规则" class="setting-card">
        <a-button type="primary" @click="addRenameRule" icon="plus">
          添加重命名规则
        </a-button>
        
        <!-- 重命名规则列表 -->
        <div class="rules-list">
          <a-collapse v-model:active-key="activeRuleKey">
            <a-collapse-panel
              v-for="(rule, index) in settings.renameRules"
              :key="index"
              :header="`规则 ${index + 1}`"
            >
              <a-form layout="vertical" :model="rule">
                <a-row :gutter="[16, 16]">
                  <a-col :span="8">
                    <a-form-item label="启用规则">
                      <a-checkbox v-model:checked="rule.enabled">启用</a-checkbox>
                    </a-form-item>
                  </a-col>
                  <a-col :span="16">
                    <a-form-item label="匹配方式">
                      <a-select v-model:value="rule.matchType" style="width: 100%">
                        ...
                        <a-select-option value="suffix">后缀匹配</a-select-option>
                ...
                <a-row :gutter="[16, 16]">
                  <a-col :span="12">
                    <a-form-item label="原工作表名">
                      <a-input
                        v-model:value="rule.oldName"
                        placeholder="输入原工作表名或匹配模式"
                      />
                    </a-form-item>
                  </a-col>
                  <a-col :span="12">
                    <a-form-item label="新工作表名">
                      <a-input
                        v-model:value="rule.newName"
                        placeholder="输入新工作表名"
                      />
                    </a-form-item>
                  </a-col>
                ...
                <div class="rule-actions">
                  <a-button type="default" @click="removeRenameRule(index)" danger>
                    删除规则
                  </a-button>
                </div>
              </a-form>
        ...
        <div class="preview-section">
          ...
          <div v-if="availableSheets.length > 0" class="sheets-list">
            <h4>工作表列表：</h4>
            <ul>
              <li v-for="sheet in availableSheets" :key="sheet">{{ sheet }}</li>
            </ul>
          </div>
        </div>
      </a-card>
...
const pluginTitle = '重命名 Excel 工作表';
...
const addRenameRule = () => {
  ...
  // 激活新添加的规则面板
  activeRuleKey.value = [String(settings.renameRules.length - 1)];
};

// 删除重命名规则
const removeRenameRule = (index) => {
  settings.renameRules.splice(index, 1);
  // 更新激活的面板
  if (activeRuleKey.value.length > 0) {
    const key = parseInt(activeRuleKey.value[0]);
    if (key >= settings.renameRules.length) {
      activeRuleKey.value = [String(Math.max(0, settings.renameRules.length - 1))];
    }
  }
};
...
const startProcess = async () => {
  ...
  try {
    // 获取Python脚本
    const script = await getPythonScript('rename-sheets');
    if (!script) {
